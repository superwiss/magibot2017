...
public class WorkerManager {
	...
	public void updateWorkerStatus() {
		...
		for (Unit worker : workerData.getWorkers()) {
			...
			if (worker.isIdle()) {
				...
				if ((workerData.getWorkerJob(worker) != WorkerData.WorkerJob.Build) && (workerData.getWorkerJob(worker) != WorkerData.WorkerJob.Move) && (workerData.getWorkerJob(worker) != WorkerData.WorkerJob.Scout)) {
	...
	public Unit chooseRepairWorkerClosestTo(Position p, int maxRange) {
		...
		for (Unit worker : workerData.getWorkers()) {
			...
			if (worker.isCompleted() && (workerData.getWorkerJob(worker) == WorkerData.WorkerJob.Minerals || workerData.getWorkerJob(worker) == WorkerData.WorkerJob.Idle || workerData.getWorkerJob(worker) == WorkerData.WorkerJob.Move)) {
	...
	public Unit getClosestResourceDepotFromWorker(Unit worker) {
		...
		for (Unit unit : MyBotModule.Broodwar.self().getUnits()) {
			...
			if (unit.getType().isResourceDepot() && (unit.isCompleted() || unit.getType() == UnitType.Zerg_Lair || unit.getType() == UnitType.Zerg_Hive)) {
				...
				else if (distance < closestDistance && workerData.depotHasEnoughMineralWorkers(unit) == false) {
					closestDepot = unit;
					closestDistance = distance;
				}
			}
