...
public class ConstructionManager {
	...
	public void checkForDeadlockConstruction() {
		...
		for (ConstructionTask b : constructionQueue) {
			if (b.getStatus() != ConstructionTask.ConstructionStatus.UnderConstruction.ordinal()) {
				...
				if (!isDeadlockCase && !InformationManager.Instance().getOccupiedRegions(InformationManager.Instance().selfPlayer).contains(desiredPositionRegion) && InformationManager.Instance().getOccupiedRegions(InformationManager.Instance().enemyPlayer).contains(desiredPositionRegion)) {
					isDeadlockCase = true;
				}
	...
	public boolean isEvolvedBuilding(UnitType type) {
		if (type == UnitType.Zerg_Sunken_Colony || type == UnitType.Zerg_Spore_Colony || type == UnitType.Zerg_Lair || type == UnitType.Zerg_Hive || type == UnitType.Zerg_Greater_Spire) {
			return true;
		}
		return false;
	}
