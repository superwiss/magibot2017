...
public class StrategyManager extends Observable {

	private static StrategyManager instance = new StrategyManager();
	private boolean isFoundFirstEnemyExpansion = false;
	...
	private void registerStrategy() {
		addObserver(new Strategy4Drone(Strategy.FourDrone));
		addObserver(new Strategy9Drone(Strategy.NineDrone));
		this.strategy = Strategy.FourDrone;
	}
	...
	public void setInitialBuildOrder() {
		if (MyBotModule.Broodwar.self().getRace() == Race.Protoss) {
			...
			BuildManager.Instance().buildQueue.queueAsLowestPriority(InformationManager.Instance().getWorkerType(), BuildOrderItem.SeedPositionStrategy.MainBaseLocation, true);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Protoss_Gateway, BuildOrderItem.SeedPositionStrategy.MainBaseLocation, true);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(InformationManager.Instance().getWorkerType(), BuildOrderItem.SeedPositionStrategy.MainBaseLocation, true);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(InformationManager.Instance().getWorkerType(), BuildOrderItem.SeedPositionStrategy.MainBaseLocation, true);
			BuildManager.Instance().buildQueue.queueAsLowestPriority(UnitType.Protoss_Zealot, BuildOrderItem.SeedPositionStrategy.MainBaseLocation, true);
	...
	public boolean initialBuildOrderFinished() {
		return isInitialBuildOrderFinished;
	}

	public boolean fullScaleAttackStarted() {
		return isFullScaleAttackStarted;
	}

	public void fullScaleAttackStarted(boolean fullAttackStarted) {
		this.isFullScaleAttackStarted = fullAttackStarted;
	}

	public boolean foundFirstEnemyExpansion() {
		return isFoundFirstEnemyExpansion;
	}

	public void foundFirstEnemyExpansion(boolean foundFirstEnemyExpansion) {
		this.isFoundFirstEnemyExpansion = foundFirstEnemyExpansion;
	}
