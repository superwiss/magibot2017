...
public class GameCommander {
	...
	public void onFrame() {
		...
		log.info(LogFormatter.info("onFrame Start"));

		try {
			if (isToFindError)
				log.warn(LogFormatter.info("Found errors on Step %d)", 1));

			// 아군 베이스 위치. 적군 베이스 위치. 각 유닛들의 상태정보 등을 Map 자료구조에 저장/업데이트
			InformationManager.Instance().update();

			if (isToFindError)
				log.warn(LogFormatter.info("Found errors on Step %d)", 2));

			// 각 유닛의 위치를 자체 MapGrid 자료구조에 저장
			MapGrid.Instance().update();

			if (isToFindError)
				log.warn(LogFormatter.info("Found errors on Step %d)", 3));
			...
			if (isToFindError)
				log.warn(LogFormatter.info("Found errors on Step %d)", 4));

			// 빌드오더큐를 관리하며, 빌드오더에 따라 실제 실행(유닛 훈련, 테크 업그레이드 등)을 지시한다.
			BuildManager.Instance().update();

			if (isToFindError)
				log.warn(LogFormatter.info("Found errors on Step %d)", 5));

			// 빌드오더 중 건물 빌드에 대해서는, 일꾼유닛 선정, 위치선정, 건설 실시, 중단된 건물 빌드 재개를 지시한다
			ConstructionManager.Instance().update();

			if (isToFindError)
				log.warn(LogFormatter.info("Found errors on Step %d)", 6));

			// 게임 초기 정찰 유닛 지정 및 정찰 유닛 컨트롤을 실행한다
			//ScoutManager.Instance().update();

			if (isToFindError)
				log.warn(LogFormatter.info("Found errors on Step %d)", 7));

			// 전략적 판단 및 유닛 컨트롤
			//StrategyManager.Instance().update();

			if (isToFindError)
				log.warn(LogFormatter.info("Found errors on Step %d)", 8));

			ControlManager.Instance().run(model);

			if (isToFindError)
				log.warn(LogFormatter.info("Found errors on Step %d)", 9));

		} catch (Exception e) {
			log.warn(LogFormatter.warnException(e));
		}

		log.info(LogFormatter.info("onFrame Finish"));
	}
	...
	public void onUnitDiscover(Unit unit) {
		...
		if (unit.getType() == UnitType.Terran_Marine) {
			model.getMyUnitManager().add(unit);
		} else if (unit.getType() == UnitType.Zerg_Zergling) {
			model.getEnemyUnitManager().add(unit);
		}
		*/

		model.addUnit(unit);
		try {
			log.debug(LogFormatter.debug("onUnitDiscover: %s", StringUtil.printUnitDebug(unit)));
		} catch (Exception e) {
			log.warn(LogFormatter.warnException(e));
		}
	}
